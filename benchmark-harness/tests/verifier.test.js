...
import { verifyAcceptanceSuite } from '../src/verifier/index.js';
...
describe('verifyAcceptanceSuite', () => {
    ...
    it('should run hidden tests in isolated directory', async () => {
        ...
        await writeFile(join(HIDDEN_DIR, 'test.cjs'), `const assert = require('node:assert/strict');
...
assert.strictEqual(1 + 1, 2, 'Math is broken');`);
        ...
        const result = await verifyAcceptanceSuite(FIXTURE_DIR, config, HIDDEN_DIR);
        assert.equal(result.passed, true);
    });
