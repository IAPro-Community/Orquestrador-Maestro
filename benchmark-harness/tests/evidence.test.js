...
import { createEvidenceDir, preserveRawEvidence, sanitizeSecrets, } from '../src/evidence/index.js';
...
describe('preserveRawEvidence', () => {
    it('creates evidence files', async () => {
        ...
        assert.ok(result.rawFiles.some((f) => f.includes('agent-output')));
        assert.ok(result.rawFiles.some((f) => f.includes('verifier-output')));
        assert.ok(result.rawFiles.some((f) => f.includes('session.json')));
    });
    it('sanitizes secrets in agent output', async () => {
        ...
        const agentFile = result.rawFiles.find((f) => f.includes('agent-output'));
        assert.ok(agentFile);
    ...
    it('writes git diff when provided', async () => {
        ...
        assert.ok(result.rawFiles.some((f) => f.includes('git-diff')));
    });
    it('writes files changed when provided', async () => {
        ...
        const fcFile = result.rawFiles.find((f) => f.includes('files-changed'));
        assert.ok(fcFile);
    ...
    it('records session metadata', async () => {
        ...
        const sessionFile = result.rawFiles.find((f) => f.includes('session.json'));
