...
describe('loadScenario', () => {
    ...
    it('should throw on non-existent file', async () => {
        await assert.rejects(() => loadScenario(join(FIXTURE_DIR, 'nonexistent.json')), (err) => {
    ...
    it('should throw on invalid JSON', async () => {
        ...
        await assert.rejects(() => loadScenario(badPath), (err) => {
...
describe('loadAllScenarios', () => {
    it('should load all valid scenarios from a directory', async () => {
        ...
        await writeFile(join(dir, 'a.json'), JSON.stringify(makeValidScenario({ id: 'scenario-a' })));
        await writeFile(join(dir, 'b.json'), JSON.stringify(makeValidScenario({ id: 'scenario-b' })));
    ...
    it('should skip non-scenario JSON files', async () => {
        ...
        await mkdir(dir, { recursive: true });
        await writeFile(join(dir, 'valid.json'), JSON.stringify(makeValidScenario({ id: 'good-one' })));
        await writeFile(join(dir, 'package.json'), JSON.stringify({ name: 'not-a-scenario' }));
        const scenarios = await loadAllScenarios(dir);
        assert.equal(scenarios.length, 1);
        assert.equal(scenarios[0].id, 'good-one');
    });
    it('should handle non-existent directory', async () => {
        await assert.rejects(() => loadAllScenarios(join(FIXTURE_DIR, 'nope')), (err) => {
    ...
    it('should recurse into subdirectories', async () => {
        ...
        await writeFile(join(dir, 'root.json'), JSON.stringify(makeValidScenario({ id: 'root-scenario' })));
        await writeFile(join(sub, 'nested.json'), JSON.stringify(makeValidScenario({ id: 'nested-scenario' })));
        const scenarios = await loadAllScenarios(dir);
        assert.equal(scenarios.length, 2);
        const ids = scenarios.map((s) => s.id).sort();
