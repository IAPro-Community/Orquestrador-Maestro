...
export function resolveIntent(description, { aliases = {}, router = { skills: {} }, chains = { chains: {} }, profiles = { profiles: {} } } = {}) {
  ...
  for (const [alias, skillId] of Object.entries(aliases)) {
    ...
    } else if (phraseMatches(lowerDesc, normalizedAlias)) {
  ...
  for (const [skillId, skill] of Object.entries(router.skills || {})) {
    for (const trigger of skill.triggers || []) {
      ...
      } else if (phraseMatches(lowerDesc, normalizedTrigger)) {
  ...
  const primarySkill = primarySkillId
    ? { id: primarySkillId, ...(router.skills?.[primarySkillId] || router.librarySkills?.[primarySkillId] || {}) }
    : null;
  ...
  if (primarySkillId && chains.chains?.[primarySkillId]) {
    const chain = chains.chains[primarySkillId];
    for (const allowedSkill of chain.mayInvoke || []) {
      if (matchedSkills.has(allowedSkill)) {
        ...
        chainedSkills.push({
          id: allowedSkill,
          ...(router.skills?.[allowedSkill] || router.librarySkills?.[allowedSkill] || {}),
  ...
  const guidedSkills = ranked
    ...
    .map(([skillId, details]) => ({
      id: skillId,
      ...(router.skills?.[skillId] || router.librarySkills?.[skillId] || {}),
      matchScore: details.score,
      matchedEvidence: details.evidence
    }));
