...
export default function Skills(){
  ...
  const items=useMemo(()=>{
    ...
    return data.skills.filter((skill)=>!needle||[skill.id,skill.description,skill.category,...skill.tags,...skill.triggers,...skill.aliases].join(' ').toLocaleLowerCase('pt-BR').includes(needle));
  },[query]);
  return <Layout title="Skills">
    <main className="shell section">
      <span className="eyebrow">CATÁLOGO GERADO</span>
      <h1 className="page-title">Skills do Maestro</h1>
      <p>Não existe lista manual nesta página. O catálogo é reconstruído de <code>SKILLS_MANIFEST.json</code>, <code>SKILLS_ROUTER.json</code> e <code>PUBLIC_SKILLS_MANIFEST.json</code> em cada deploy.</p>
      ...
      <div className="skill-grid">{items.map((skill)=><article key={skill.id}>
        ...
        <h3>{skill.id}</h3><p>{skill.description}</p>
