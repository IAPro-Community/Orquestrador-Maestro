...
class MaestroTreeProvider {
  ...
  async getChildren(element) {
    try {
      ...
      if (element.key === "runs") { const runs = await this.client.call("runs.list", { projectPath: this.workspacePath }); return runs.length ? runs.map((run) => { const resolution = run.metadata?.resolution; const state = resolution?.outcome?.state || run.status; const strategy = resolution?.strategy ? ` · ${resolution.strategy}` : ""; return item(`${state}${strategy}: ${run.id}`, 0, { command: "maestro.showRun", title: "Show Run", arguments: [run.id] }); }) : [item("Nenhum Run neste projeto")]; }
...
function openCockpit(context, client, workspacePath) {
  ...
  panel.webview.html = `<!doctype html><html><head><link rel="stylesheet" href="${xtermStyle}"><style>body{margin:0;background:#05070b;color:#e6edf5;font:13px system-ui}.cockpit{display:grid;grid-template-columns:300px 1fr;min-height:100vh}.nav{padding:16px;background:#0a0f16;border-right:1px solid #243244}.grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;padding:12px}.pane{min-height:180px;border:1px solid #243244;background:#0a0f16}.pane:first-child{grid-row:span 2}.meta{padding:7px;color:#8391a5;border-bottom:1px solid #162131}.terminal{height:145px}.resolution{margin-top:14px;padding-top:12px;border-top:1px solid #243244;line-height:1.65;color:#aab7c8}.resolution strong{color:#e6edf5}</style></head><body><div class="cockpit"><aside class="nav"><h3>◆ Maestro</h3><div id="project"></div><p id="mission"></p><div id="resolution" class="resolution"></div></aside><main><div class="meta">Piloto e especialistas · até seis terminais por página</div><section id="grid" class="grid"></section></main></div><script src="${xtermScript}"></script><script>const api=acquireVsCodeApi(),terminals=new Map(),sequences=new Map();function render(d){document.querySelector('#project').textContent=d.project.name+' · '+d.project.status;document.querySelector('#mission').textContent=d.mission?.objective||'Nenhuma missão ativa';const r=d.project.resolution;document.querySelector('#resolution').innerHTML=r?'<strong>Resolution</strong><br>State: '+(r.state||'—')+'<br>Strategy: '+(r.strategy||'—')+'<br>Mode: '+(r.mode||'—')+'<br>Budget: '+(r.budget?.tier||'—')+(Number.isFinite(r.budget?.contextTokens)?' · '+r.budget.contextTokens+' ctx tokens':'')+'<br>Evidence: '+(r.evidence?.selected?.length??0)+'/'+(r.evidence?.candidates??0)+'<br>Escalation: '+(r.escalation?.count??0)+'/'+(r.escalation?.max??0)+'<br>Verification: '+(d.project.verification?.status||'—')+(r.outcome?.reason?'<br>Outcome: '+r.outcome.reason:''):'<strong>Resolution</strong><br>Sem Run canônico ainda.';for(const s of d.sessions.filter(x=>x.backend==='pty').slice(0,6)){let p=document.getElementById(s.id);if(!p){p=document.createElement('div');p.id=s.id;p.className='pane';p.innerHTML='<div class="meta"></div><div class="terminal"></div>';document.querySelector('#grid').append(p);const t=new Terminal({convertEol:true,scrollback:2000,theme:{background:'#0a0f16'}});t.open(p.querySelector('.terminal'));t.onData(input=>api.postMessage({type:'input',terminalId:s.id,input}));terminals.set(s.id,t);sequences.set(s.id,0)}p.querySelector('.meta').textContent=(s.providerId||'shell')+' · '+s.label+' · '+s.status;api.postMessage({type:'snapshot',terminalId:s.id,afterSequence:sequences.get(s.id)||0})}}window.addEventListener('message',e=>{if(e.data.type==='dashboard')render(e.data.data);if(e.data.type==='snapshot'){const d=e.data.data,t=terminals.get(d.session.id);if(t&&d.deltaAnsi)t.write(d.deltaAnsi);sequences.set(d.session.id,d.sequence||0)}});api.postMessage({type:'ready'});</script></body></html>`;
