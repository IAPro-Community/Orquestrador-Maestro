...
exports.cmdInspect = cmdInspect;
exports.cmdPreflight = cmdPreflight;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const node_child_process_1 = require("node:child_process");
const harness_1 = require("./harness");
const schema_1 = require("./harness/schema");
const BENCHMARKS_DIR = node_path_1.default.join(__dirname);
function parseArgs(args) {
    ...
    while (i < args.length) {
        ...
        if (arg === "--model") {
            ...
            parsed.conditions = args[++i].split(",");
        }
        else if (arg === "--output") {
...
function cmdList(options) {
    const scenarios = (0, harness_1.listScenarios)();
    ...
    if (options.json) {
        const items = scenarios.map((id) => {
            const scenario = (0, harness_1.loadScenario)(id);
...
function cmdValidate(scenarioId, options = {}) {
    const scenarioIds = scenarioId ? [scenarioId] : (0, harness_1.listScenarios)();
    ...
    for (const id of scenarioIds) {
        try {
            const scenario = (0, harness_1.loadScenario)(id);
            const result = (0, schema_1.validateScenario)(scenario);
            results.push({ id, valid: result.valid, errors: result.errors });
            if (result.valid) {
                if (!options.json)
                    console.log(`✓ Scenario "${id}" is valid.`);
            }
            else {
                valid = false;
                if (!options.json) {
            ...
            valid = false;
            results.push({ id, valid: false, errors: [error.message] });
            if (!options.json)
...
async function cmdRun(options) {
    const scenarios = options.scenarios || (0, harness_1.listScenarios)();
    if (scenarios.length === 0) {
    ...
    try {
        const results = await (0, harness_1.runBenchmark)(scenarios, options);
        console.log(`\nCompleted ${results.length} run(s).`);
        const report = (0, harness_1.generateReport)(results, options);
        const outputDir = options.outputDir || node_path_1.default.join(BENCHMARKS_DIR, "results");
        if (!node_fs_1.default.existsSync(outputDir)) {
            node_fs_1.default.mkdirSync(outputDir, { recursive: true });
        }
        const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        const jsonPath = node_path_1.default.join(outputDir, `report-${timestamp}.json`);
        const mdPath = node_path_1.default.join(outputDir, `report-${timestamp}.md`);
        node_fs_1.default.writeFileSync(jsonPath, report.json, "utf8");
        node_fs_1.default.writeFileSync(mdPath, report.markdown, "utf8");
        if (options.json) {
            outputJson({
                completed: true,
...
function cmdCompare(file1, file2, options) {
    if (!file1 || !file2) {
        console.error("Error: two result files required. Usage: compare <file1> <file2>");
        process.exit(1);
    }
    try {
        const r1 = JSON.parse(node_fs_1.default.readFileSync(file1, "utf8"));
        const r2 = JSON.parse(node_fs_1.default.readFileSync(file2, "utf8"));
        if (options.json) {
            outputJson({
                file1: node_path_1.default.basename(file1),
                file2: node_path_1.default.basename(file2),
                result1: r1,
                result2: r2,
            });
            return;
        ...
        console.log(`  File 1: ${node_path_1.default.basename(file1)}`);
        console.log(`  File 2: ${node_path_1.default.basename(file2)}\n`);
...
function cmdSuite(options) {
    const profile = options.profile || "official";
    let scenarios = (0, harness_1.listScenarios)();
    if (options.filter) {
        scenarios = scenarios.filter((id) => {
            const s = (0, harness_1.loadScenario)(id);
...
function cmdReport(runSet, options) {
    if (!runSet) {
    ...
    try {
        const files = node_fs_1.default.readdirSync(outputDir).filter((f) => f.endsWith(".json") && f.startsWith(`report-${runSet}`));
        if (files.length === 0) {
        ...
        for (const file of files) {
            const report = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(outputDir, file), "utf8"));
            if (report.results) {
        ...
        const report = (0, harness_1.generateReport)(allResults, options);
        if (format === "json") {
...
function cmdVerify(runId, options) {
    if (!runId) {
        console.error("Error: run-id required. Usage: verify <run-id>");
        process.exit(1);
    }
    const resultsDir = options.outputDir || node_path_1.default.join(BENCHMARKS_DIR, "results");
    try {
        ...
        console.error(`Error verifying run: ${error.message}`);
        process.exit(1);
    }
}
function cmdInspect(runId, options) {
    if (!runId) {
    ...
    try {
        ...
        console.error(`Error inspecting run: ${error.message}`);
        process.exit(1);
    }
}
function cmdPreflight(options) {
    ...
    // Docker
    try {
        (0, node_child_process_1.execSync)("docker --version", { encoding: "utf8", stdio: "pipe" });
    ...
    // OpenCode
    try {
        const ver = (0, node_child_process_1.execSync)("opencode --version", { encoding: "utf8", stdio: "pipe" }).trim();
    ...
    // Git
    try {
        const gitVer = (0, node_child_process_1.execSync)("git --version", { encoding: "utf8", stdio: "pipe" }).trim();
...
function loadRunById(resultsDir, runId) {
    if (!node_fs_1.default.existsSync(resultsDir))
        return null;
    const files = node_fs_1.default.readdirSync(resultsDir).filter((f) => f.endsWith(".json"));
    for (const file of files) {
        const report = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(resultsDir, file), "utf8"));
